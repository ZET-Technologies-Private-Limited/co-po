...
export default function AdminThresholdsPage() {
  ...
  const rows: ParamRow[] = [
    { key: "targetPassPct", label: "Target Pass%", value: thresholds.targetPassPct, scope: "Global" },
    ...
    { key: "cieWeight", label: "CIE Weightage%", value: thresholds.cieWeight, scope: "Global" },
    { key: "seeWeight", label: "SEE Weightage%", value: thresholds.seeWeight, scope: "Global" },
    { key: "minCOs", label: "Min COs", value: thresholds.minCOs, scope: "Regulation Year" },
    { key: "maxCOs", label: "Max COs", value: thresholds.maxCOs, scope: "Regulation Year" },
    { key: "absentPolicy", label: "Absent Student Policy", value: thresholds.absentPolicy, scope: "Global" },
  ];
  ...
  function saveEdit(row: ParamRow) {
    ...
    const patch = row.key === "absentPolicy" ? { [row.key]: value as "include" | "exclude" } : { [row.key]: Number(draftValue) };
    setThresholds(patch);
    setHistory((prev) => [
      {
        id: `${Date.now()}-${row.key}`,
