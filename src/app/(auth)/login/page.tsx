...
export default function LoginPage() {
  ...
  // Lockout state (persisted in sessionStorage for demo)
  ...
  const DEPT_LOOKUP: Record<string, string> = {
    "admin@nexus.edu": "Administration",
    "hod@nexus.edu": "CSE",
    "lead@nexus.edu": "CSE",
    "faculty@nexus.edu": "CSE",
    "student@nexus.edu": "CSE",
  };

  const handleEmailChange = (val: string) => {
    setEmail(val);
    const found = DEPT_LOOKUP[val.trim()];
  ...
  const fillDemo = (creds: typeof DEMO_CREDS[0]) => {
    ...
    const found = DEPT_LOOKUP[creds.email];
    if (found) setDept(found);
    setFieldErrors({});
  };
              ...
              >
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {process.env.NODE_ENV !== "production" && (
                    <button
                      onClick={() => { setLocked(false); setAttempts(0); sessionStorage.removeItem("login_attempts"); sessionStorage.removeItem("login_locked"); }}
                      className="text-xs text-white/30 hover:text-white/60 transition-colors font-mono text-left"
                    >
                      (Demo: reset lockout)
                    </button>
                  )}
