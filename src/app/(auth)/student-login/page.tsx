...
export default function StudentLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Resolve student email from roll number
    let allUsers: any[] = [];
    try {
      const stored = localStorage.getItem("obe-ai-data-store");
      allUsers = stored ? JSON.parse(stored)?.state?.users || [] : [];
    } catch {}
    if (!allUsers.length) allUsers = users;

    const studentUser = allUsers.find(
      (u: any) => u.employeeId === roll.trim() && u.roles?.includes("student")
    );
    ...
    if (!studentUser) {
      ...
      setFieldErrors({ roll: "No student account found with this roll number" });
      setIsLoading(false);
      return;
    }

    const success = login(studentUser.email, password);
    if (success) {
  ...
  const handlePwChange = async (e: React.FormEvent) => {
    ...
    if (currentUser) {
      ...
      if (dbUser) updateUser(dbUser.id, { password: newPw, firstLogin: false });
      ...
      <div className="hidden lg:flex flex-col justify-between w-5/12 bg-gradient-to-b from-cyan-950/40 to-blue-950/20 border-r border-white/5 px-16 py-16">
        ...
        <div className="flex flex-col gap-2 text-[10px] font-mono text-white/20 uppercase tracking-widest">
          ...
          <div className="flex flex-col gap-1 text-white/40">
            <span>Roll: <span className="text-cyan-400">21CS001</span></span>
            <span>Password: <span className="text-cyan-400">Student@1</span></span>
          </div>
        </div>
      </div>
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="w-full max-w-sm">
          ...
          {locked ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-6">
              ...
              <button
                onClick={() => { setLocked(false); setAttempts(0); sessionStorage.removeItem("sl_attempts"); sessionStorage.removeItem("sl_locked"); }}
                className="text-xs text-white/20 hover:text-white/40 transition-colors font-mono text-left"
              >
                (Demo: reset lockout)
              </button>
            </motion.div>
